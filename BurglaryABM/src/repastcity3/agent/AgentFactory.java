 ...
 * </pre>
 * 
 * Create 'num_agents' agents in randomly chosen houses. The agents are of type <code>BurglarAgent</code>. For example,
...
public class AgentFactory {
	...
	public AgentFactory(String agentDefinition) throws AgentCreationException {
		...
		if (split.length != 3) {
			throw new AgentCreationException("Problem parsing the definition string '" + agentDefinition
		...
		String defn2 = split[2]; // Number of police
	...
	private void createRandomAgents(boolean dummy) throws AgentCreationException {
		...
		// The definition has been parsed OK, now can either stop or create the agents
		if (dummy) {
			return;
		}
		...
		while (agentsCreated < numAgents) {			
			...
			while (i.hasNext() && agentsCreated < numAgents) {
				...
				//TODO PICK A SMARTER BUILDING CHOICE BASED ON CENSUS DATA
				if (b.getBtype() == 1 && b.stress >= 3){
					IAgent a = new BurglarAgent(); // Create a new agent
					a.setHome(b); // Tell the agent where it lives
					a.setMoneyIncrementLevel(); // Initialize agent's money level
		...
		int numPolice = Integer.parseInt(this.definition2);
		// Create police in randomly chosen houses. Use two while loops in case there are more agents
		// than houses, so that houses have to be looped over twice.
		// LOGGER.info("Creating " + numPolice + " police agents using " + this.methodToUse + " method.");
		int policeCreated = 0;
		int jurisdiction = 1;
		while (policeCreated < numPolice) {
			//LOGGER.info("policeCreated = " + policeCreated);
			Iterator<Building> i = ContextManager.buildingContext.getRandomObjects(Building.class, numPolice).iterator();
			while (i.hasNext() && policeCreated < numPolice) {
				//LOGGER.info("policeCreated = " + policeCreated);
					Building b = i.next(); // Find a police station
				if (b.getBtype() == 5 && b.jurisdiction == jurisdiction) {
					IAgent a = new PoliceAgent(); // Create a new agent //change to new agent
					a.setHome(b); // Assign agent to police station
					a.setJurisdiction(b);
					a.setAgentType(2);
					b.addAgent(a); // Tell the police station the agent works there
					ContextManager.addAgentToContext(a); // Add the agent to the context
					// Finally move the agent to the place where it works.
					ContextManager.moveAgent(a, ContextManager.buildingProjection.getGeometry(b).getCentroid());
					policeCreated++;
					jurisdiction++;
					if (jurisdiction > 3){
